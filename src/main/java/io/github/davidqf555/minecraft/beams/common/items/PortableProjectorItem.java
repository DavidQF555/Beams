...
public class PortableProjectorItem extends ShootableItem {
    ...
    public void releaseUsing(ItemStack stack, World world, LivingEntity entity, int duration) {
        ...
        if (range > 0) {
            BeamEntity beam = BeamEntity.shoot(EntityRegistry.BEAM.get(), world, entity.getEyePosition(1), entity.getLookAngle(), range, ProjectorInventory.getModuleTypes(ProjectorInventory.get(stack)), 0.5, 0.5, 0.5, 0.5, null);
            if (beam != null) {
                beam.setLifespan(20);
