...
public abstract class WhitelistTargetingModuleItem extends TargetingModuleItem {
    ...
    private final static Component BLACKLIST_TEXT = Component.translatable("item." + Beams.ID + ".whitelist_targeting_module.blacklist").withStyle(ChatFormatting.GREEN);
    private final static Component WHITELIST_TEXT = Component.translatable("item." + Beams.ID + ".whitelist_targeting_module.whitelist").withStyle(ChatFormatting.RED);

    public WhitelistTargetingModuleItem(Properties properties) {
        super(properties.component(WHITELIST, false));
    }
