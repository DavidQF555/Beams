...
public class Cuboid {
    ...
    public static Cuboid fromBeam(BeamEntity beam) {
        ...
        sides[0] = new ConvexQuadrilateral3D(new Vec3[][]{{vertices[0], vertices[2]}, {vertices[1], vertices[3]}}, center.reverse());
        sides[1] = new ConvexQuadrilateral3D(new Vec3[][]{{vertices[4], vertices[6]}, {vertices[5], vertices[7]}}, center);
        sides[2] = new ConvexQuadrilateral3D(new Vec3[][]{{vertices[0], vertices[7]}, {vertices[3], vertices[4]}}, vertices[4].subtract(vertices[0]).cross(horizontal.reverse()).normalize());
        sides[3] = new ConvexQuadrilateral3D(new Vec3[][]{{vertices[2], vertices[5]}, {vertices[1], vertices[6]}}, vertices[6].subtract(vertices[2]).cross(horizontal).normalize());
        sides[4] = new ConvexQuadrilateral3D(new Vec3[][]{{vertices[2], vertices[7]}, {vertices[3], vertices[6]}}, vertices[6].subtract(vertices[2]).cross(vertical.reverse()).normalize());
        sides[5] = new ConvexQuadrilateral3D(new Vec3[][]{{vertices[4], vertices[1]}, {vertices[0], vertices[5]}}, vertices[4].subtract(vertices[0]).cross(vertical).normalize());
    ...
    public void doBlockEffect(Consumer<BlockPos> effect) {
        int startY = Mth.floor(getBounds().minY);
    ...
    public boolean isColliding(AABB bounds) {
        ...
        for (ConvexQuadrilateral3D side : sides) {
            for (Vec3 start : side.vertices[0]) {
                for (Vec3 end : side.vertices[1]) {
    ...
    public boolean isColliding(Vec3 pos) {
        for (ConvexQuadrilateral3D side : sides) {
            Vec3 dir = side.vertices[0][0].subtract(pos).normalize();
    ...
    private ConvexPolygon2D[] calculateSlices() {
        ...
        ConvexPolygon2D[] slices = new ConvexPolygon2D[Mth.floor(bounds.maxY) - Mth.floor(bounds.minY) + 1];
        double[] heights = Arrays.stream(vertices).mapToDouble(Vec3::y).sorted().distinct().toArray();
        int index = 0;
        for (int i = 0; i < slices.length; i++) {
            // will be sorted, but can have duplicates
            List<Double> crit = new ArrayList<>();
            int low = Mth.floor(bounds.minY) + i;
            if (low >= bounds.minY) {
                crit.add((double) low);
            }
            int high = low + 1;
    ...
    private static class ConvexPolygon2D {
        ...
        public void doEffect(BiConsumer<Integer, Integer> effect) {
            int index = 0;
            for (int x = Mth.floor(this.x[0]); x <= this.x[this.x.length - 1]; x++) {
