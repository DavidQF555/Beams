...
public abstract class DirectedProjectorBlock extends AbstractProjectorBlock {
    ...
    public List<BeamEntity> shoot(Level world, BlockPos pos, BlockState state) {
        ...
        BeamEntity beam = BeamEntity.shoot(EntityRegistry.BEAM.get(), world, start, dir, ServerConfigs.INSTANCE.projectorMaxRange.get(), getModules(world, pos, state), size, size, null, pos);
        if (beam == null) {
